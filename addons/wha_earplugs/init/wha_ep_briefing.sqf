...
if !_cbaPresent then
{
	...
	_briefingString = _briefingString + "TOGGLE KEY:   ( <execute expression=""hintsilent format['Earplugs can be toggled in and out with your %1 key.',WHA_EP_TOGGLE_NAME];WHA_EP_TOGGLE=true;[false] call compile preprocessFileLineNumbers 'wh\addons\ep\init\wha_ep_toggleKey.sqf';profileNamespace setVariable ['WHA_EP_TOGGLE',true];saveProfileNamespace;"">YES</execute> / <execute expression=""hintsilent format['The %1 key will not be used for earplugs.',WHA_EP_TOGGLE_NAME];WHA_EP_TOGGLE=false;call compile preprocessFileLineNumbers 'wh\addons\ep\init\wha_ep_toggleKeyDestroy.sqf';profileNamespace setVariable ['WHA_EP_TOGGLE',false];saveProfileNamespace;"">NO</execute> ) 
	( Default: NO )" +
